...
const sendTemporalyPassword = (to, name, password) => {
  const pathname = generatePathName("registerAuth");
  const file = fs
    .readFileSync(pathname, { encoding: "utf-8" })
    .toString()
    .replace("${password_temporal}", password);

  send(to, file, "¡Shoe Kingdom te da la Bienvenida!");
};

export default { sendEmail, saleNotification, sendTemporalyPassword };
